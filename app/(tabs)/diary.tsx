...
export default function DiaryListView() {
  ...
  // hardware back should go to Home from this tab
  useEffect(() => {
    const onBack = () => {
      router.push("/");
      return true;
  ...
  const handleCreateEntry = async () => {
    ...
    router.push({
      pathname: "/diary-page",
      params: { id: newEntry.id, title: newEntry.title },
    });
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      />
      <Text style={[styles.title, { color: colors.text }]}>My Pages</Text>
...
const DiaryCard = ({
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    const extraTranslate = Math.min(
      expandAmount.value * 0.8 * (index / Math.max(1, total - 1)),
      500,
    );
  ...
  return (
    <Animated.View style={animatedStyle}>
      <Pressable
        style={[
          styles.entryCard,
          { backgroundColor: item.color, zIndex: 100, elevation: 5 },
        ]}
        onPress={onPress}
