...
export default function DiaryPageScreen() {
  ...
  // Hardware back should return to diary list first
  useEffect(() => {
    const onBack = () => {
      router.push("/diary");
      return true;
  ...
  const [imageMenuElementId, setImageMenuElementId] = useState<string | null>(
    null,
  );
  const [isAddingText, setIsAddingText] = useState(false);
  const [modalMode, setModalMode] = useState<"text" | "sticky" | "sticker">(
    "text",
  );
  ...
  const [redoStack, setRedoStack] = useState<
    { elements: DiaryElement[]; paths: DiaryElement[] }[]
  >([]);
  ...
  const pushToHistory = React.useCallback(
    (newElements: DiaryElement[], newPaths: DiaryElement[]) => {
      setHistory((prev) => [
        ...prev.slice(-19),
        { elements: [...newElements], paths: [...newPaths] },
      ]);
      setRedoStack([]);
    },
    [],
  ...
  const finalizePath = React.useCallback(
    async (pointsToSave: { x: number; y: number }[]) => {
    ...
    [id, activeTool, elements, paths, pushToHistory],
  );

  const drawPan = React.useMemo(
    () =>
      Gesture.Pan()
        .enabled(isDrawMode)
        .onStart((e) => {
    ...
    [isDrawMode, activePointsSV, cursorPos, finalizePath, isDrawing],
  );
  ...
  const handleUndo = async () => {
    ...
    // Persist to storage
    await saveDiaryElements(
      [...previousState.elements, ...previousState.paths],
      id,
    );
  };
      ...
      {isDrawMode ? (
        <View style={styles.drawingToolbar}>
          ...
          >
            <IconSymbol
              name="pencil.tip"
              size={24}
              color={activeTool === "pencil" ? "#fff" : "#333"}
            />
          </Pressable>
          <Pressable
            style={[
              styles.toolButton,
              activeTool === "pen" && styles.activeTool,
            ]}
        ...
        <View style={styles.fabContainer}>
          <Pressable
            ...
            style={[
              styles.fab,
              { backgroundColor: theme === "dark" ? "#000000" : "#B2CEFE" },
            ]}
          >
          ...
          <Pressable
            style={[
              styles.fab,
              { backgroundColor: theme === "dark" ? "#000000" : "#FFD1DC" },
            ]}
          ...
          <Pressable
            style={[
              styles.fab,
              { backgroundColor: theme === "dark" ? "#000000" : "#FFF9C4" },
            ]}
            onPress={() => openAddModal("sticky")}
          >
          ...
          <Pressable
            style={[
              styles.fab,
              { backgroundColor: theme === "dark" ? "#000000" : "#C8E6C9" },
            ]}
            onPress={() => openAddModal("sticker")}
          >
          ...
          <Pressable
            style={[
              styles.fab,
              { backgroundColor: theme === "dark" ? "#000000" : "#D1C4E9" },
            ]}
            onPress={addImage}
          >
              ...
              <View style={styles.pickerSection}>
                ...
                <View style={styles.styleContainer}>
                  {FONTS.map((font) => (
                    ...
                    >
                      <Text
                        style={[styles.fontOptionText, { fontFamily: font }]}
                      >
                        A
                      </Text>
                    </Pressable>
            ...
            <View style={styles.modalButtons}>
              ...
              >
                <Text style={styles.buttonText}>
                  {editingElementId ? "Save Changes" : "Add to Page"}
                </Text>
              </Pressable>
            </View>
          </View>
...
const DraggableElement = ({
  ...
  const startSize = useSharedValue({
  ...
  const pan = React.useMemo(
    ...
    [element, onUpdate, disabled, isPressed, offset, start, rotation, scale],
  );
  ...
  const resizePan = React.useMemo(
        ...
        .onUpdate((e) => {
          if (element.type === "image" || element.type === "sticker") {
            ...
            const newWidth = Math.max(
              50,
              startSize.value.width + e.translationX,
            );
    ...
    [element, onUpdate, disabled, isPressed, startSize, width, height],
  );
    ...
    return {
      ...
      height:
        element.type === "image" || element.type === "sticker"
          ...
          : undefined,
      minHeight:
        element.type === "text" || element.type === "sticky" ? 100 : undefined,
    };
  });
  ...
  const cycleFont = () => {
    ...
    const currentIndex = FONTS.indexOf(
      element.fontFamily || "PatrickHand_400Regular",
    );
    ...
    <Animated.View style={animatedStyle}>
      ...
      {showImageMenu && element.type === "image" && (
        <Animated.View style={[styles.imageMenuContainer, controlStyle]}>
          ...
          >
            <IconSymbol
              name={element.hasShadow ? "sun.max.fill" : "shadow"}
              size={20}
              color="#FF8FAB"
            />
            <Text style={styles.imageMenuText}>
              {element.hasShadow ? "Remove Shadow" : "Add Shadow"}
            </Text>
          </Pressable>
          <Pressable
            onPress={() => {
        ...
        <Animated.View style={[styles.badgeContainer, controlStyle]}>
          ...
          {element.type === "text" && (
            <Pressable
              ...
              style={[
                styles.deleteBadge,
                { backgroundColor: "#448AFF", right: -40 },
              ]}
            >
