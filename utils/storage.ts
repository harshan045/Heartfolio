...
export type Polaroid = {
  ...
  width?: number; // Image width for aspect ratio detection
  height?: number; // Image height for aspect ratio detection
  orientation?: 'vertical' | 'horizontal'; // User-selected orientation
};
...
export const MAGNET_COLORS = ['#ff9191', '#9fc2fe', '#fff6a8', '#a1ffd1', '#dad9db', '#ffb350'];
...
const ALBUM_ORDER_KEY = 'heartfolio_album_order';
...
export const saveMemory = async (newMemory: Polaroid): Promise<void> => {
  try {
    ...
    const index = existingMemories.findIndex(m => m.id === newMemory.id);
    let updatedMemories;
    if (index >= 0) {
      updatedMemories = [...existingMemories];
      updatedMemories[index] = newMemory;
    } else {
      updatedMemories = [...existingMemories, newMemory];
    }
...
export const getAlbums = async (): Promise<string[]> => {
  try {
    ...
    const albumSet = new Set(memories.map(m => m.album));
    const allAlbums = Array.from(albumSet);
    
    // Get stored order
    const orderJson = await getStorageData(ALBUM_ORDER_KEY);
    const storedOrder: string[] = orderJson ? JSON.parse(orderJson) : [];
    
    // Sort by stored order, then add any new albums at the end
    const orderedAlbums = [
      ...storedOrder.filter(a => allAlbums.includes(a)),
      ...allAlbums.filter(a => !storedOrder.includes(a))
    ];
...
export const renameAlbum = async (oldName: string, newName: string): Promise<void> => {
  try {
    ...
    if (orderJson) {
      ...
      if (index >= 0) {
        order[index] = newName;
        await AsyncStorage.setItem(getScopedKey(ALBUM_ORDER_KEY), JSON.stringify(order));
      }
    }
  } catch (error) {
...
export const deleteAlbum = async (albumName: string): Promise<void> => {
  try {
    // 1. Delete memories in this album
    const memoriesJson = await getStorageData(STORAGE_KEY);
    if (memoriesJson) {
      const memories: Polaroid[] = JSON.parse(memoriesJson);
      const updatedMemories = memories.filter(m => m.album !== albumName);
      await AsyncStorage.setItem(getScopedKey(STORAGE_KEY), JSON.stringify(updatedMemories));
    }

    // 2. Delete paper bits in this album
    const bitsJson = await getStorageData(PAPER_BITS_KEY);
    if (bitsJson) {
      const bits: PaperBit[] = JSON.parse(bitsJson);
      const updatedBits = bits.filter(b => b.album !== albumName);
      await AsyncStorage.setItem(getScopedKey(PAPER_BITS_KEY), JSON.stringify(updatedBits));
    }

    // 3. Remove from album order
    const orderJson = await getStorageData(ALBUM_ORDER_KEY);
    if (orderJson) {
      const order: string[] = JSON.parse(orderJson);
      const filteredOrder = order.filter(a => a !== albumName);
      await AsyncStorage.setItem(getScopedKey(ALBUM_ORDER_KEY), JSON.stringify(filteredOrder));
    }
  } catch (error) {
...
export const updateAlbumOrder = async (newOrder: string[]): Promise<void> => {
  try {
    await AsyncStorage.setItem(getScopedKey(ALBUM_ORDER_KEY), JSON.stringify(newOrder));
  } catch (error) {
